...
## Aktuálny stav projektu (2026-04-19, Sprint 3 + 3.5 hotfix saga SHIPPED, Task #18 regression gate LIVE)
...
- `build-with-docker/local-urcap-api/api-1.3.0.jar` — **real URCap API JAR** (Universal Robots A/S, 168 KB, z URCap SDK 1.18). Spolu s `.pom` a `-sources.jar` priamo v repo — Dockerfile `install:install-file` ho inštaluje do build-container `~/.m2`. Commitnuté v rámci Task #15, `2026-04-19`. **Toto je first-line-of-defense** proti stub drift — druhá úroveň je regression gate, ale bez reálneho JAR-u by sme stále museli dôverovať hand-rolled stubom.
